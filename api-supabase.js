...
(function (global) {
  ...
  if (!global.supabase || !global.SUPABASE_URL || !global.SUPABASE_ANON_KEY) {
    ...
    return; // e.g. offline: the CDN script did not load. index.html shows "No Network".
  }
  ...
  function isNetworkError(e) {
    ...
    return /no network|failed to fetch|networkerror|network request failed|load failed|fetch failed|internet_disconnected/i
  ...
  async function requireSession() {
    ...
    if (error && isNetworkError(error)) throw noNetworkError();
    if (error || !session) {
  ...
  async function uploadFile(bucket, path, file, contentType) {
    const { error } = await sb.storage.from(bucket).upload(path, file, {
      upsert: true,
      // Some phones give an empty file.type; passing it explicitly keeps PDFs/images openable
      contentType: contentType || file.type || undefined
    });
    if (error) {
      const m = String(error.message || '');
      if (/row-level security|policy|not authorized|unauthorized/i.test(m)) {
        throw new Error('Upload not allowed. Check the storage policies for the "' + bucket + '" bucket.');
      }
      if (/maximum allowed size|too large|payload/i.test(m)) {
        throw new Error('File is too large to upload (max 5 MB).');
      }
    ...
    // private: store path; signed URL generated when needed (see getLearnerDocumentUrl)
    return path;
  }

  // Turn a stored value (plain path, or an old public/signed URL) into a bucket path
  function storagePath(bucket, pathOrUrl) {
    if (!pathOrUrl) return pathOrUrl;
    const markers = [`/object/public/${bucket}/`, `/object/sign/${bucket}/`, `/object/authenticated/${bucket}/`];
    for (const marker of markers) {
      const idx = pathOrUrl.indexOf(marker);
      if (idx >= 0) return decodeURIComponent(pathOrUrl.slice(idx + marker.length).split('?')[0]);
    }
    return pathOrUrl;
  }
  ...
  function newStoragePath(file, fallbackExt) {
    const ext = (file.name.split('.').pop() || fallbackExt).toLowerCase().replace(/[^a-z0-9]/g, '') || fallbackExt;
    return `${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
  }

  // Upload the profile picture and/or PDF at the same time. If either fails, anything that
  // did upload is removed so no orphan files are left behind.
  async function uploadLearnerFiles(profileFile, docFile) {
    const out = { profile: null, doc: null };
    const jobs = [];
    if (profileFile && profileFile.size) {
      jobs.push(uploadFile('learner-profiles', newStoragePath(profileFile, 'jpg'), profileFile)
        .then(v => { out.profile = v; }));
    }
    if (docFile && docFile.size) {
      jobs.push(uploadFile('learner-documents', newStoragePath(docFile, 'pdf'), docFile, 'application/pdf')
        .then(v => { out.doc = v; }));
    }
    const results = await Promise.allSettled(jobs);
    const failed = results.find(r => r.status === 'rejected');
    if (failed) {
      await removeUploadedLearnerFiles(out);
      throw failed.reason instanceof Error ? failed.reason : new Error('Upload failed');
    }
    return out;
  }

  async function removeUploadedLearnerFiles(up) {
    if (!up) return;
    if (up.profile) await removeStoragePath('learner-profiles', up.profile);
    if (up.doc) await removeStoragePath('learner-documents', up.doc);
  }
  ...
  async function resolveUserRole(user) {
    ...
    try {
      ...
      if ([profRes, teachRes].some(function (r) { return r && r.error && isNetworkError(r.error); })) {
      ...
      if (e && e.isNetwork) throw e; // otherwise keep the old "best effort" behaviour
    }
  ...
  const api = {
    ...
    async login(usernameOrEmail, password) {
      ...
      if (navigator.onLine === false) throw noNetworkError();
      let res;
      try {
        ...
        if (isNetworkError(e)) throw noNetworkError();
        throw e;
      }
      const { data, error } = res;
      if (error) {
        if (isNetworkError(error)) throw noNetworkError();
    ...
    async createLearner(formData) {
      ...
      // uploads (profile + document in parallel)
      const profileFile = formData.get('profilePicture');
      const docFile = formData.get('document');
      const up = await uploadLearnerFiles(profileFile, docFile);
      if (up.profile) body.profile_picture = up.profile;
      if (up.doc) {
        body.document_path = up.doc;
        body.document_name = docFile.name;
      }

      const { data, error } = await sb.from('learners').insert(body).select('id').single();
      if (error) {
        await removeUploadedLearnerFiles(up);
    ...
    async updateLearner(id, formData) {
      ...
      const profileFile = formData.get('profilePicture');
      const docFile = formData.get('document');
      // Upload new files first; old ones are only removed once the row is saved
      const up = await uploadLearnerFiles(profileFile, docFile);
      const toRemove = [];
      if (up.profile) {
        toRemove.push(['learner-profiles', current.profile_picture]);
        body.profile_picture = up.profile;
      }
      if (up.doc) {
        toRemove.push(['learner-documents', current.document_path]);
        body.document_path = up.doc;
        body.document_name = docFile.name;
      } else if (formData.get('clearDocument') === '1' || formData.get('clearDocument') === 'true') {
        toRemove.push(['learner-documents', current.document_path]);
      ...
      for (const [bucket, p] of toRemove) await removeStoragePath(bucket, p);
      return { message: 'Updated' };
    },

    // Private bucket: build a short-lived signed link so the PDF can be opened/downloaded
    async getLearnerDocumentUrl(id) {
      await requireSession();
      const { data: row, error } = await sb.from('learners').select('document_path, document_name').eq('id', id).maybeSingle();
      if (error) throw new Error(error.message);
      if (!row || !row.document_path) throw new Error('No document attached to this learner');
      const name = row.document_name || 'document.pdf';
      const path = storagePath('learner-documents', row.document_path);
      if (/^https?:/i.test(path)) return { url: path, name };
      const { data, error: sErr } = await sb.storage.from('learner-documents').createSignedUrl(path, 300, { download: name });
      if (sErr || !data || !data.signedUrl) {
        const m = (sErr && sErr.message) || '';
        throw new Error(/not found/i.test(m) ? 'Document file was not found in storage. Please upload it again.' : (m || 'Could not create download link'));
      }
    ...
    async getTeachers() {
      await requireSession();
      const { data, error } = await sb.from('teachers').select('id, first_name, middle_name, surname, tsc_no, id_no, phone, email, tsc_email, gender, date_of_birth, date_joined, roles, is_class_teacher, assigned_class, education_level, teaching_level, subject_combination, learning_areas, work_load').order('surname');
      if (error) throw new Error(error.message);
      return (data || []).map((r) => ({
        ...r,
    ...
    // (phone, TSC Gmail). Everything else stays admin-only via
    ...
    async updateOwnTeacherProfile(body) {
      ...
      const row = { phone, tsc_email: tscEmail };
  ...
  async function apiFetch(url, opts = {}) {
    // Offline: fail fast with a clear message (sign-out is still allowed to clear local state)
    if (url !== '/api/logout' && typeof navigator !== 'undefined' && navigator.onLine === false) {
      throw noNetworkError();
    }
    try {
      return await routeRequest(url, opts);
    } catch (e) {
      if (isNetworkError(e)) throw noNetworkError();
      if (e && (e.status === 401 || /session expired|not authenticated/i.test(e.message || ''))) {
        try {
  ...
  async function routeRequest(url, opts) {
...

    {
      ...
      if (url === '/api/login' && method === 'POST') {
        return await api.login(body.username, body.password);
      }
      ...
      if (url === '/api/verify-password' && method === 'POST') {
        return await api.verifyPassword(body.password);
      }

      // Learners
      if (url === '/api/learners' && method === 'GET') {
        return await api.getLearners();
      }
      if (url === '/api/learners' && method === 'POST') {
        return await api.createLearner(body); // FormData
      }
      if (/^\/api\/learners\/[^/]+\/document$/.test(url) && method === 'GET') {
        return await api.getLearnerDocumentUrl(url.split('/')[3]);
      }
      if (url.startsWith('/api/learners/') && method === 'PUT') {
        const id = url.split('/').pop();
        return await api.updateLearner(id, body);
      }
      if (url.startsWith('/api/learners/') && method === 'DELETE') {
        const id = url.split('/').pop();
        return await api.deleteLearner(id);
      }
      if (url === '/api/learners/promote' && method === 'POST') {
        return await api.promoteLearners(body.fromClass, body.password, body.passwordConfirm);
      }
      ...
      if (url.startsWith('/api/grades') && method === 'GET') {
        ...
        return await api.getGrades(q);
      }
      if (url === '/api/grades' && method === 'POST') {
        // Bulk: { term, exam, year, entries: [...] }
        if (body && Array.isArray(body.entries)) {
          return await api.saveGradesBulk(body);
        }
        return await api.saveGrades(body);
      }
      ...
      if (url === '/api/teachers' && method === 'POST') {
        return await api.createTeacher(body);
      }
      if (url === '/api/teachers/me' && method === 'PUT') {
        return await api.updateOwnTeacherProfile(body);
      }
      if (url.startsWith('/api/teachers/') && method === 'PUT') {
        const id = url.split('/').pop();
        return await api.updateTeacher(id, body);
      }
      if (url.startsWith('/api/teachers/') && method === 'DELETE') {
        const id = url.split('/').pop();
        return await api.deleteTeacher(id);
      }
