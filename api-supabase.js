...
(function (global) {
  ...
  const api = {
    ...
    async login(usernameOrEmail, password) {
      // Accept email directly, or treat username as local-part of email
      let email = String(usernameOrEmail || '').trim();
      if (email && !email.includes('@')) {
        // fallback: if user typed old-style username, try common pattern
        email = email + '@kagumori.local';
      }
    ...
    async me() {
      ...
      return {
        user: {
          ...
          name: (rows && rows.name) || 'Administrator',
          role: (rows && rows.role) || 'admin'
        }
      };
    },
    ...
    async saveGrades({ learnerId, term, exam, year, scores }) {
      await requireSession();
      const session = await requireSession();
      const cols = ['eng', 'math', 'kisw', 'ss', 'cas', 'agri', 'ins', 'pre_tech', 'cre'];
      const byCol = Object.fromEntries(cols.map((c) => [c, 0]));
  ...
  function mapTeacherBody(body) {
    return {
      ...
      learning_areas: (body.learningAreas || []).map((a) =>
        typeof a === 'string'
          ? { name: a.trim(), class: null }
          : { name: String(a.name || '').trim(), class: a.class || null }
      )
    };
  }
