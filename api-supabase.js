...
(function (global) {
  ...
  // localStorage: survives refresh, tab switches, and opening another app on mobile.
  // sessionStorage is wiped when the OS discards a background page — that caused logout.
  // Explicit Sign Out still clears the session.
    ...
    try {
      if (email) {
        const { data: trow } = await sb
          .from('teachers')
          .select('first_name, middle_name, surname, email')
          .ilike('email', email)
          .maybeSingle();
        if (trow) {
  ...
  const api = {
    ...
    async login(usernameOrEmail, password) {
      ...
      if (error) {
        ...
        // Supabase often returns the same message for wrong password and unconfirmed email
        if (/email not confirmed|email_not_confirmed/i.test(raw)) {
          throw new Error(
            'Email not confirmed. In Supabase Dashboard → Authentication → Users, open this account and confirm the email (or turn off “Confirm email” under Providers → Email).'
          );
        }
        if (/invalid login credentials|invalid_credentials|invalid credentials/i.test(raw)) {
          throw new Error(
            'Invalid credentials. Teachers: use the email registered for them and the TSC No as the password (digits only, no spaces). If this is a new teacher, open Supabase → Authentication → Users, confirm their email, and ensure the password matches the TSC No.'
          );
        }
    ...
    try {
      ...
      if (e.status === 401 || /session expired|not authenticated/i.test(e.message || '')) {
        try {
          ...
          localStorage.removeItem('kagumori_session'); // clear any legacy flag
        } catch (_) {}
      }
      throw e;
    }
