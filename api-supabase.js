...
(function (global) {
  ...
  const api = {
    ...
    async getTeachers() {
      await requireSession();
      const { data, error } = await sb.from('teachers').select('id, first_name, middle_name, surname, tsc_no, id_no, phone, email, tsc_email, gender, date_of_birth, date_joined, bank_acc_no, roles, is_class_teacher, assigned_class, education_level, teaching_level, subject_combination, learning_areas').order('surname');
      if (error) throw new Error(error.message);
      return (data || []).map((r) => ({
        ...r,
